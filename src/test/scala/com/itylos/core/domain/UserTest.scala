...
class UserTest extends WordSpecLike with Matchers with BeforeAndAfterEach with ScalatestRouteTest {
  ...
  val userMobiles = JField("phones", new JArray(List(new JString("mobile"))))
  ...
  "A User" must {
    "throw exception when user id is missing and it's required" in {
      val jObject = JObject(List(userName, userEmail, userMobiles, userWebPass, userAlarmPass, userIdAdmin))
      val user = new User()
    ...
    "throw exception when user name is missing " in {
      val jObject = JObject(List(userOId, userEmail, userMobiles, userWebPass, userAlarmPass, userIdAdmin))
      val user = new User()
      intercept[ParameterMissingException] {
        user.fromJObject(jObject, isIdRequired = true)
      }
    ...
    "throw exception when user email is missing" in {
      val jObject = JObject(List(userOId, userName, userMobiles, userWebPass, userAlarmPass, userIdAdmin))
      val user = new User()
      intercept[ParameterMissingException] {
        user.fromJObject(jObject, isIdRequired = true)
      }
    }
    "throw exception when user mobiles are missing" in {
    ...
    "throw exception when user web pass is missing" in {
      val jObject = JObject(List(userOId, userName, userEmail, userMobiles, userAlarmPass, userIdAdmin))
      val user = new User()
      intercept[ParameterMissingException] {
        user.fromJObject(jObject, isIdRequired = true)
      }
    ...
    "throw exception when user alarm pass is missing" in {
      val jObject = JObject(List(userOId, userName, userEmail, userMobiles, userWebPass, userIdAdmin))
      val user = new User()
      intercept[ParameterMissingException] {
        user.fromJObject(jObject, isIdRequired = true)
      }
    ...
    "throw exception when user is admin is missing" in {
      val jObject = JObject(List(userOId, userName, userEmail, userMobiles, userWebPass, userAlarmPass))
      val user = new User()
      intercept[ParameterMissingException] {
        user.fromJObject(jObject, isIdRequired = true)
      }
    ...
    "setup user from json data" in {
      val jObject = JObject(List(userOId, userName, userEmail, userMobiles, userWebPass, userAlarmPass, userIdAdmin))
      val user = new User()
      user.fromJObject(jObject, isIdRequired = true)
      user.oid shouldBe Some("oid")
      user.name shouldBe "name"
      user.email shouldBe "email"
      user.phones shouldBe List("mobile")
